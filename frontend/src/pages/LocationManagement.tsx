...
const LocationManagement: React.FC = () => {
  ...
  const [assigningLocation, setAssigningLocation] = useState<{
    storeId: number;
    locationId: string;
    locationName: string;
    storeName: string;
  ...
  const createMappingMutation = useMutation({
    mutationFn: async ({
      ...
      const response = await api.post(
        `/location-aliases/${aliasId}/mappings`,
        mapping,
      );
      return response.data;
    },
  ...
  const handleDeleteAlias = (id: number) => {
    if (
      confirm(
        "Are you sure you want to delete this alias and all its mappings?",
      )
    ) {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <motion.div
        ...
        className="mb-8"
      >
        <h1 className="text-3xl font-bold text-gray-900 dark:text-dark-800 mb-4">
          Location Management
        </h1>
        <p className="text-gray-600 dark:text-dark-500">
          Assign aliases to your Shopify fulfillment locations for consistent
          rule management across all your stores.
        </p>
      </motion.div>
      ...
      >
        <h2 className="text-xl font-semibold text-gray-900 dark:text-dark-800 mb-6">
          Fulfillment Locations from Connected Stores
        </h2>
        ...
        {storeLocations && storeLocations.length > 0 ? (
          <div className="space-y-6">
            {storeLocations.map((store) => (
              <div
                key={store.store_id}
                className="border border-gray-200 dark:border-dark-200 rounded-lg p-4"
              >
                <div className="mb-4">
                  <h3 className="font-medium text-gray-900 dark:text-dark-800">
                    {store.store_name}
                  </h3>
                  ...
                  <div className="space-y-3">
                    {store.locations.map((location) => {
                      ...
                      return (
                        <div
                          key={location.id}
                          className="flex items-center justify-between p-3 bg-gray-50 dark:bg-dark-50 rounded"
                        >
                          <div>
        ...
        <div className="space-y-4">
            ...
            >
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`badge ${alias.is_active ? "badge-success" : "badge-gray"}`}
                  >
              ...
              <div className="bg-gray-50 dark:bg-dark-200 p-3 rounded">
                <div className="text-sm text-gray-600 dark:text-dark-500 mb-2">
                  Mapped to {alias.mappings.length} store
          ...
          {aliases?.length === 0 && (
            <div className="text-center py-8 text-gray-500">
              No location aliases created yet. Create your first alias to get
              started.
            </div>
          )}
        </div>
          ...
          <div className="bg-white dark:bg-dark-100 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="mb-4 p-3 bg-gray-50 rounded">
              <div className="text-sm text-gray-600">
                Store: {assigningLocation.storeName}
              </div>
              <div className="font-medium">
                {assigningLocation.locationName}
              </div>
              <div className="text-xs text-gray-500">
                {assigningLocation.locationId}
              </div>
            </div>
            ...
            <div className="space-y-4 mb-6">
              <div>
                <label className="label">
                  Choose existing alias or create new
                </label>
                <select
                  value=""
                  onChange={(e) => {
            ...
            <div className="flex gap-2">
              {newAliasName !== undefined && (
                <button
                  ...
                  disabled={
                    !newAliasName.trim() || createAliasMutation.isPending
                  }
        ...
        <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-dark-100 rounded-lg p-6 max-w-md w-full mx-4">
            ...
            <div className="flex gap-2">
              <button
                onClick={() =>
                  handleUpdateAlias(editingAlias, {
