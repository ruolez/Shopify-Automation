...
def run_migration():
    ...
    try:
        with engine.begin() as conn:
            ...
            if not check_table_exists(conn, 'fraud_analyses_archive'):
                logger.info("fraud_analyses_archive table does not exist - nothing to do")
                return
            
            # Check if the column exists
            column_exists = check_column_exists(conn, 'fraud_analyses_archive', 'age_checker_detected')
            ...
            if column_exists:
                ...
                if db_type == "postgresql":
                    ...
                    conn.execute(text("ALTER TABLE fraud_analyses_archive DROP COLUMN IF EXISTS age_checker_detected"))
                    logger.info("Successfully removed age_checker_detected column from fraud_analyses_archive (PostgreSQL)")
                else:
                ...
                """
                    ...
                    logger.info("Successfully removed age_checker_detected column from fraud_analyses_archive (SQLite)")
            else:
