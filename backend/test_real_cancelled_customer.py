"""Test fraud analysis with a real customer who has cancelled orders"""
...
async def test_real_cancelled_customer():
    """Test fraud analysis for John Kelley's recent order PW110573"""
    ...
    try:
        ...
        # Test with John Kelley's most recent order
        test_order = "PW110573"
        logger.info(f"Testing order: {test_order}")
        logger.info("Customer: John Kelley (has cancelled order PW19925 in history)")
        ...
        if fraud_analysis:
            ...
            if saved_analysis:
                ...
                if saved_analysis.previous_order_cancelled is True:
                    ...
                    logger.info("   A customer with a cancelled order in their history was properly flagged.")
                else:
                    logger.error("\n❌ PROBLEM: The field was not set to True despite cancelled order in history")
...
if __name__ == "__main__":
    asyncio.run(test_real_cancelled_customer())